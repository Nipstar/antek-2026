...
- Visible order (conversion-first): HERO (benefit H1 incl. town + subhead + Book a free 30-min call + click-to-call `tel:03330389960`) → QuickRecap → PROBLEM → OFFER (3 core services as outcomes + single CTA, H2 = "AI automation for [Town] businesses") → TrustBlock → "Our [Town] patch" entity layer → ResourcesCompliance → sibling/geo cards (up to hub + 3 siblings) → **GEO cross-sell block** ("Is AI Search Recommending Your [Town] Business?" → localised ChatGPT/Perplexity query → link to `/services/geo-audit`, from £247) → FAQ (5 inline `<details>`) → final CTA → CalBooking → VoiceChat modal.
